...
def dynamics(t, x, u):
    ...
    h = -m2*l1*lc2*sin(q2)
    C = array([[h*dq2*(2*dq1 + dq2)], [-h*dq1**2]])
    G = array([[g*(m1*lc1 + m2*l1)*sin(q1) + g*m2*lc2*sin(q1+q2)],
         [g*m2*lc2*sin(q1+q2)]])
    B = array([[b1*dq1], [b2*dq2]])
    
    # Accelerations
    tau = array([[u], [0]])  # Only first joint actuated
    detM = M11*M22 - M12**2
    ddq1 = (M22*(tau[0] - C[0] - G[0] - B[0]) - M12*(tau[1] - C[1] - G[1] - B[1])) / detM
    ddq2 = (M11*(tau[1] - C[1] - G[1] - B[1]) - M12*(tau[0] - C[0] - G[0] - B[0])) / detM
    
    xdot = array([[dq1], [dq2], [ddq1], [ddq2]])
    return xdot