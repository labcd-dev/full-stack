...
from backend_core.Trimmer.agenticNodes.tools.tools import (
    compute_jacobian,
    analyze_stability,
    validate_constraints
)
from backend_core.Trimmer.agenticNodes.agents import Agents
import numpy as np
import json
...
def analyze_result(state: WorkflowState, writer) -> WorkflowState:
    ...
    """
    writer({"progress": 0.9, "text": "🧐 Analysing Result ..."})
    ...
    if state.get('converged'):
        output_text += "\n\n  ✓ Converged! Performing linearization and stability analysis..."
        ...
        if match:
            if desired_check_skipped:
                output_text += "\n✓ No desired operating conditions provided; skipping match check."
            else:
                output_text += "\n✓ Equilibrium matches desired operating conditions!"
            if analysis:
                output_text += f"\n    {analysis}"
            state['restart_count'] = 0
        else:
            output_text += "\n  ✗ Equilibrium does NOT match desired operating conditions."
            if analysis:
                output_text += f"\n    {analysis}"
    ...
    if state['ui_mode'] == 'streamlit':
        writer({"agent_tag": "🧐.Equilibrium Check", "log_history": output_text})
    else:
        print(output_text)
...
def handle_human_intervention(state: WorkflowState, failure_type: str, writer) -> WorkflowState:
    ...
    writer({"progress": 0.9, "text": "🤖 Human Intervention ..."})

    logger = state['logger']
    logger.info(f"--- Entering Node: handle_human_intervention ({failure_type}) ---")

    output_text = f"\n\n🤖 {failure_type}. Invoking human intervention..."
    ...
    output_text += f"\n\n🤖 HumanInterventionAgent: Analyzing workflow failure..."
    ...
    if action in valid_actions:
        ...
        output_text += f"\n  📋 Diagnosis: {root_cause} (confidence: {conf_txt})"
        ...
        output_text += "\n  ⚠ LLM analysis unavailable/invalid. Using default action: ask_new_desired_conditions."
    ...
    if should_exit:
        ...
        output_text += "\n\n🔄 Continuing workflow with updated inputs..."
        state['restart_count'] = 0
    ...
    if state['ui_inputs'] == 'streamlit':
        writer({"agent_tag": "🤖.Human Intervention", "log_history": output_text})
    else:
        print(output_text)
...
def _execute_human_intervention_action(action, root_cause, explanation, operating_conditions, initial_guess, bounds, output_text):
    ...
    if (not interactive_session) and action in {
        ...
        output_text += "\n\n⚠ Non-interactive session detected. Manual intervention inputs are unavailable."
    ...
    if action == "exit":
        if root_cause == 'model_problem':
            output_text += "\n\n❌ The model has a problem. Terminating the process."
        elif root_cause == 'best_possible_solution':
            output_text += "\n\n✅ Accepting best possible solution. Terminating the process."
        else:
            output_text += "\n\n❌ User chose to exit workflow."
        result['exit'] = True
        return result, output_text
    ...
    if action == "ask_new_desired_conditions":
        output_text += "\n\n🔄 Restarting from parser to specify new desired operating conditions..."
        result['new_inputs']['restart_from_parser'] = True
        return result, output_text

    if action == "ask_new_initial_values":
        output_text += "\n\n❓ Let's update the initial guess values."
        ...
        if user_input:
            try:
                ...
                output_text += "\n   ✓ Updated initial guess to: {new_guess}"
            except ValueError:
    ...
    if action == "ask_new_states":
        output_text += "\n\n❓ Let's update the state configuration."
        ...
        if confirm == 'y':
            ...
            if config_path:
                ...
                result['new_inputs']['restart_from_parser'] = True
                output_text += "\n   ✓ Will restart from parser with new config: {config_path}"
            else:
                output_text += "\n   ⚠ No path provided. Keeping current configuration."
        else:
            output_text += "\n   → Keeping current state configuration."
        return result, output_text

    if action == "ask_new_bounds":
        output_text += "\n\n❓ Let's update the bounds for state and input variables."
        output_text += "\n   Current bounds: {json.dumps(bounds, indent=2)}"
                ...
                if user_input:
                    try:
                        ...
                        if len(new_values) == len(current_values):
                            ...
                            output_text += "\n     ✓ Updated {bound_type} to: {new_values}"
                        else:
                            output_text += "\n     ⚠ Expected {len(current_values)} values, got {len(new_values)}. Keeping current."
                    except ValueError:
                        output_text += "\n     ⚠ Invalid input format. Keeping current {bound_type}: {current_values}"
                else:
                    output_text += "\n     → Keeping {bound_type}: {current_values}"

        result['new_inputs']['bounds'] = new_bounds
        output_text += "\n\n   ✓ Bounds updated successfully!"
        return result, output_text

    if action == "retry":
        output_text += "\n\n🔄 Retrying with current inputs."
        return result, output_text

    output_text += "\n\n⚠ Unknown action: {action}. Retrying with current inputs."
    result['action'] = 'retry'
    return result, output_text
