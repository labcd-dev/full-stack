...
class LLMActor(LLMBaseAgent):
        ...
        if 'reasoning' in params and 'Fallback' in params['reasoning']:
            for attempt in range(max_retries):
                ...
                if 'reasoning' in params and 'Fallback' not in params['reasoning']:
                    return params
                log_to_file(f"❗️Retry {attempt + 1}/{max_retries}: Failed to parse response", True)
    ...
    def _parse_generalized_response(self, response, controller_type, param_ranges, system):
        ...
        try:
            ...
            log_to_file(f"❗️Error parsing actor response: {e}", True)
...
"""
        ...
        try:
            ...
            log_to_file(f"❗️Error parsing critic response: {e}", True)
            fallback_feedback = {
...
class LLMTerminator(LLMBaseAgent):
    ...
    def judge_termination(self, buffer, current_metrics, target_metrics, max_iterations, controller_type,
        ...
        try:
            ...
            log_to_file(f"❗️Error parsing termination JSON: {e}", True)
            backup_decision = {
                "decision": "CONTINUE",
                "reasoning": f"❗️Error parsing response: {str(e)}. Continuing by default.",
