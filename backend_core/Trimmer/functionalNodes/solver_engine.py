...
from backend_core.Trimmer.agenticNodes.tools.tools import solve_equilibrium as solve_equilibrium_tool


def plan_strategy(state: WorkflowState, writer) -> WorkflowState:
    """Node to plan the solving strategy using YAML-driven agent."""
    writer({"progress": 0.5, "text": "🧠 Planing Strategy..."})
    ...
    if state["ui_mode"] == "streamlit":
        writer({"agent_tag": "🧠.Strategy Planner", "log_history": output_text})
    else:
        print(output_text)
...
def solve_equilibrium_node(state: WorkflowState, writer) -> WorkflowState:
    ...
    writer({"progress": 0.8, "text": "👨‍🏫 Solving Equation..."})
    ...
    if x_e is not None and u_e is not None:
        ...
        try:
            ...
            output_text += f"\n  ✗ Error: Failed to convert equilibrium values to numeric: {e}"

    if converged:
        output_text += f"\n  ✓ Converged!"
    else:
        output_text += f"\n  ✗ Failed to converge after {max_retries} attempts. Best cost: {best_cost:.2e}"
    ...
    if state["ui_mode"] == "streamlit":
        writer({"agent_tag": "👨‍🏫.Equation Solver", "log_history": output_text})
    else:
        print(output_text)
...
def _adapt_solver_with_llm(state, config, old_guess, x_e, u_e, attempt, strategy, best_cost, last_cost, logger, output_text):
    ...
    try:
        output_text += f"\n    🧠 Consulting LLM for adaptive strategy (attempt {attempt + 1})..."
        ...
        output_text += f"\n   → LLM suggested strategy: {new_strategy}"
        if reasoning:
            output_text += f"\n    → Reasoning: {reasoning}"
        ...
        if "rate_limit_exceeded" in str(e) or "Rate limit reached" in str(e):
            output_text += f"\n    ⚠ Rate limit reached. Stopping adaptive retries."
            raise e

        # Fallback to heuristic adaptation
        output_text += f"\n   ⚠ LLM call failed: {e}. Using heuristic adaptation."
