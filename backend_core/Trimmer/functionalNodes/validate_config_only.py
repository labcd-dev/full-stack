...
from backend_core.Trimmer.services.human_input import require_human_input
...
def validate_config_only(state: WorkflowState, writer) -> WorkflowState:
    ...
    if not config.get('param_vars'):
        print("\n⚠ No parameters identified. Please provide system parameters.")
    else:
        ...
        if specifiable:
            ...
            if user_choice:
                try:
                    ...
                    print(f"  ⚠ Invalid input. Using first {limit} items.")
    ...
    if not config.get('bounds') or not all(k in config['bounds']
        ...
        print("\n⚠ Bounds not fully specified. Please provide bounds.")
