...
from backend_core.Trimmer.agenticNodes.create_agent import create_agent
...
class ParsingGraph:
    ...
    def _validate_config(self, state: GraphState) -> GraphState:
        ...
        if not config.get('operating_conditions') or not does_have_operating_point:
            print("\n⚠ Operating conditions are missing or incomplete.")
                ...
                if does_have_operating_point or not config.get('operating_conditions'):
                    ...
                    if user_choice:
                        try:
                            ...
                            print(f"  ⚠ Invalid input. Using first {min(len(specifiable), adjusted_n_u)} items.")
