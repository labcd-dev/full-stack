...
class Agents(metaclass=SingletonMeta):
    ...
    def control_loop_analyser(self, state, writer):
        writer({"progress": 0.5, "text": "ðŸ‘¨â€ðŸ”§ Analyzing control loop structure..."})
        ...
        writer({"agent_tag": "ðŸ‘¨â€ðŸ”§.Control Loop Analysis", "log_history": reasoning})
    ...
    def control_loop_structure(self, state, writer):
        writer({"progress": 0.75, "text": "ðŸ§± Putting results in standard format ..."})
        ...
        writer({"agent_tag": "ðŸ”.Control Loop", "log_history": final_json})
        return {"messages": [final_json], "control_loop_analysis": final_json}



    def control_loop_supervisor(self, state, writer):
        writer({"progress": 0.8, "text": "ðŸ§ Supervising and validating control loop..."})
        ...
        writer({"agent_tag": "ðŸ§.Supervisor", "log_history": response_content})
    ...
    def openai_image_recognition(self, state, writer):
        writer({"progress": 0.5, "text": "ðŸ‘€ Image Recognition ..."})
        ...
        writer({"agent_tag": "ðŸ¤–.Image Recognition Result", "log_history": response_content})
