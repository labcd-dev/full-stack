...
M_DIR = PROJECT_ROOT / "test/m"
PY_DIR = PROJECT_ROOT / "test/py"
...
m_files = list(M_DIR.glob("*.m"))
print(M_DIR)


@pytest.mark.parametrize("m_file", m_files, ids=lambda f: f.stem)
def test_matlab_to_numpy(m_file):
    # Locate the corresponding .py expected output file
    py_file = PY_DIR / f"{m_file.stem}.py"
    ...
    actual_py_content = matlab_to_numpy(matlab_content)
