...
from backend_api.Regularizer.MatlabToPython.translation_rules import RULES_DICT
...
def _convert_fprintf_in_code(code: str) -> str:
    ...
    while i < len(code):
        ...
        # ----- convert format string to fâ€‘string -----
        # Extract quoted string content
        qmatch = re.match(r'^\s*([\'"])(.*?)\1\s*$', fmt_str, re.DOTALL)
        if not qmatch:
            # Not a proper string literal â€“ fallback
            if target == "print":
        ...
        # Build the fâ€‘string body by replacing %-specifiers with {var}
        var_index = 0
        def replacer(m):
            nonlocal var_index
